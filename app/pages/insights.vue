  ...
  <div class="min-h-screen bg-slate-950 relative overflow-hidden">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[70vh] gap-6">
      ...
      <div class="text-center">
        ...
        <p class="text-xs text-slate-700 mt-1">This may take up to 30 seconds</p>
      </div>
    </div>
    ...
    <main v-else class="relative max-w-7xl mx-auto px-6 pt-10 pb-24 space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-extrabold text-white tracking-tight">Your Insights</h1>
          <p class="text-sm text-slate-600 mt-0.5">AI analysis of your spending patterns and financial behaviors</p>
        </div>
        <div class="hidden sm:flex items-center gap-2 text-xs text-slate-600 bg-white/[0.03] border border-white/[0.07] rounded-xl px-4 py-2">
          ...
          {{ documentCount }} statement{{ documentCount !== 1 ? 's' : '' }} analyzed
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="col-span-2 lg:col-span-1 relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 overflow-hidden group hover:border-emerald-500/30 hover:bg-emerald-500/[0.04] transition-all duration-300">
          ...
          <div class="flex items-center justify-between mb-3">
            <span class="text-[10px] font-bold text-slate-600 uppercase tracking-widest">HabitWealth Score</span>
        ...
        <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 overflow-hidden hover:border-red-500/20 hover:bg-red-500/[0.03] transition-all duration-300">
          ...
          <div class="flex items-center justify-between mb-3">
            <span class="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Stress Index</span>
          ...
          <p class="text-[11px] text-slate-600 mt-2">Financial stress level</p>
        </div>
        ...
        <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 overflow-hidden hover:border-pink-500/20 hover:bg-pink-500/[0.03] transition-all duration-300">
          ...
          <div class="flex items-center justify-between mb-3">
            <span class="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Total Spent</span>
          ...
          <p class="text-[11px] text-slate-600 mt-2">Last 3 months</p>
        </div>
        ...
        <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 overflow-hidden hover:border-cyan-500/20 hover:bg-cyan-500/[0.03] transition-all duration-300">
          ...
          <div class="flex items-center justify-between mb-3">
            <span class="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Impulse Control</span>
          ...
          <p class="text-[11px] text-slate-600 mt-2">Control over impulse buys</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6">
            ...
            <h2 class="text-sm font-bold text-white mb-5">Spending by Category</h2>
            ...
            <div v-else class="text-sm text-slate-600">No category data yet.</div>
          </div>
          ...
          <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6">
            ...
            <div class="flex items-center justify-between mb-5">
              <h2 class="text-sm font-bold text-white">Saving Goals</h2>
              ...
              <div v-for="(g, idx) in goals" :key="idx" class="rounded-xl border p-4 transition-colors"
                ...
                <div class="grid grid-cols-2 gap-2 mb-3">
                  <div class="rounded-lg bg-white/[0.03] px-3 py-2">
                    <p class="text-[10px] text-slate-600 mb-0.5">Saving now</p>
                    <p class="text-sm font-bold text-white">€{{ g.currentSavings }}<span class="text-xs text-slate-600 font-normal">/mo</span></p>
                  </div>
                  <div class="rounded-lg bg-white/[0.03] px-3 py-2">
                    <p class="text-[10px] text-slate-600 mb-0.5">Need to save</p>
                    <p class="text-sm font-bold text-white">€{{ g.monthlyNeeded }}<span class="text-xs text-slate-600 font-normal">/mo</span></p>
                  </div>
                </div>
                ...
                <p v-if="g.projectedMonths" class="text-[11px] text-slate-600">Projected: <span class="text-slate-400 font-medium">{{ g.projectedMonths }} months</span></p>
                <p v-else class="text-[11px] text-slate-700 italic">No savings detected this period</p>
              </div>
            </div>
            <div v-else class="text-sm text-slate-600">
              No goals set yet.
              <NuxtLink to="/get-started" class="text-emerald-400 hover:text-emerald-300 underline underline-offset-2 ml-1 transition-colors">Add goals →</NuxtLink>
            </div>
          </div>
          ...
          <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6">
            <div class="absolute inset-x-0 top-0 h-px rounded-t-2xl bg-gradient-to-r from-transparent via-white/10 to-transparent"></div>
            <h2 class="text-sm font-bold text-white mb-5">Recent Transactions</h2>
            ...
            <div v-else class="text-sm text-slate-600">No transactions analyzed yet.</div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="relative rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.05] p-6 overflow-hidden">
            ...
            <p class="text-[10px] font-bold text-emerald-700 uppercase tracking-widest mb-3">Your Financial Persona</p>
            <div class="text-xl font-extrabold text-emerald-300 mb-2 leading-tight">{{ financialPersona }}</div>
            <p class="text-xs text-slate-600 leading-relaxed">Based on spending patterns and behavioral AI analysis</p>
          </div>
          ...
          <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6">
            ...
            <h3 class="text-sm font-bold text-white mb-4">💡 Key Recommendations</h3>
            ...
            <div v-else class="text-xs text-slate-600 leading-relaxed">Upload more statements to get personalized recommendations.</div>
          </div>

          <!-- Quick stats -->
          <div class="relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6">
            <div class="absolute inset-x-0 top-0 h-px rounded-t-2xl bg-gradient-to-r from-transparent via-white/10 to-transparent"></div>
            <h3 class="text-sm font-bold text-white mb-4">Quick Stats</h3>
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <span class="text-xs text-slate-600">Statements analyzed</span>
                <span class="text-xs font-bold text-white">{{ documentCount }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-slate-600">Categories tracked</span>
                <span class="text-xs font-bold text-white">{{ topCategories.length }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-slate-600">Goals set</span>
                <span class="text-xs font-bold text-white">{{ goals.length }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-slate-600">Recommendations</span>
